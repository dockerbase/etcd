...
run:
	docker run --name $(subst /,-,$(NAME)) --restart=always -t --cidfile cidfile -p 4001:4001 -p 7001:7001 -d $(NAME):$(VERSION) /sbin/runit
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) bash -lc " go version " | tee -a COMPONENTS
	docker run -it --rm $(NAME):$(VERSION) bash -lc " ls -d /usr/local/etcd-* | cut -d / -f 4 | tr - ':' " | tee -a COMPONENTS
	dos2unix COMPONENTS
